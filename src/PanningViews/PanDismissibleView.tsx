...
class PanDismissibleView extends PureComponent<Props, State> {
  ...
  onLayout = (event: LayoutChangeEvent) => {
    if (this.height === 0) {
      ...
      this.thresholdY = threshold?.y ?? layout.height / 2;
      this.width = layout.width;
      this.thresholdX = threshold?.x ?? layout.width / 2;
      this.initPositions();
    }
  };
  ...
  resetPosition = () => {
    ...
    if (toX !== undefined) {
      animations.push(
        Animated.spring(this.animTranslateX, {
    ...
    if (toY !== undefined) {
      animations.push(
        Animated.spring(this.animTranslateY, {
  ...
  getDismissAnimationDirection = () => {
    ...
    if (hasHorizontalSwipe || hasVerticalSwipe) {
      ...
      const hasHorizontalDrag = dragDirections.x !== undefined;
      const hasVerticalDrag = dragDirections.y !== undefined;
  ...
  _animateDismiss = (isRight?: boolean, isDown?: boolean) => {
    ...
    if (toX !== undefined) {
      animations.push(
        Animated.timing(this.animTranslateX, {
          toValue: Math.round(toX),
          useNativeDriver: true,
          duration,
        })
    ...
    if (toY !== undefined) {
      animations.push(
        Animated.timing(this.animTranslateY, {
          toValue: Math.round(toY),
          useNativeDriver: true,
          duration,
        })
